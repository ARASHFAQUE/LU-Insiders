...
class ProfileSearchWidget extends StatefulWidget {
  ...
  @override
  State<ProfileSearchWidget> createState() => _ProfileSearchWidgetState();
}

class _ProfileSearchWidgetState extends State<ProfileSearchWidget> {
  ...
  Future<ChatRoomModel?> getChatRoomModel(String targetUser) async{
    ...
    QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('chats')
        .doc(widget.currentUser).collection('chat-rooms')
        .where("participants.$targetUser", isEqualTo: true)
        .where("participants.${widget.currentUser}", isEqualTo: true)
        .get();
    ...
    if(snapshot.docs.isNotEmpty){
      ...
      await FirebaseFirestore.instance.collection('chats').doc(widget.currentUser).collection('chat-rooms').doc(newChatroom.chatRoomId)
        ...
        trailing: widget.currentUser.toString() == widget.userID.toString()
          ...
          onPressed: () async{
            ...
            if(chatRoomModel != null){
              ...
              Navigator.push(context, MaterialPageRoute(builder: (context) => ChatRoomScreen(currentUser: widget.currentUser, targetUser: widget.userID, chatroom: chatRoomModel, name: widget.name, profilePic: widget.profilePic)));
            }
            //
            //_mailTo();
          },
